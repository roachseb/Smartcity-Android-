...
import com.example.samir.accueil.Adapter.ListSourceAdapter;
import com.example.samir.accueil.Common.Common;
...
public class NewsActvity extends AppCompatActivity {
   ...
   /* public String getCity(){
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activyty_news);
        //Toast.makeText(getApplicationContext(), getCity()+"", Toast.LENGTH_SHORT).show();
        //Init cache
        Paper.init( this);
    ...
    private void loadWebsiteSource(boolean isRefreshed) {
        if(!isRefreshed)
        {
            ...
            else // If not have cache
            {
                dialog.show();
                //Fetch new data
                mService.getSources(Common.getSourcesUrl("","fr","fr")).enqueue(new Callback<WebSite>() {
                    @Override
                    public void onResponse(Call<WebSite> call, Response<WebSite> response) {
                        Log.d("FIRST ARTICLE WEB :",response.body().toString());
